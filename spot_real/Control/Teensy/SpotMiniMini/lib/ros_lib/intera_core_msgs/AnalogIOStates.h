#ifndef _ROS_intera_core_msgs_AnalogIOStates_h
#define _ROS_intera_core_msgs_AnalogIOStates_h
...
namespace intera_core_msgs
{
  ...
  class AnalogIOStates : public ros::Msg
  {
    public:
      ...
      typedef intera_core_msgs::AnalogIOState _states_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(states_lengthT > states_length)
        this->states = (intera_core_msgs::AnalogIOState*)realloc(this->states, states_lengthT * sizeof(intera_core_msgs::AnalogIOState));
      states_length = states_lengthT;
      for( uint32_t i = 0; i < states_length; i++){
      offset += this->st_states.deserialize(inbuffer + offset);
        memcpy( &(this->states[i]), &(this->st_states), sizeof(intera_core_msgs::AnalogIOState));
      }
     return offset;
    }

    const char * getType(){ return "intera_core_msgs/AnalogIOStates"; };
